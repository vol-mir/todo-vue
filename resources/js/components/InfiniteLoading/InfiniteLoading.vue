...
<script>
import Spinner from '@components/InfiniteLoading/Spinner.vue'
import config, {
  evt3rdArg, WARNINGS, STATUS, SLOT_STYLES
} from '@components/InfiniteLoading/config'
import {
  warn, throttleer, loopTracker, scrollBarStorage, kebabCase, isVisible
} from '@components/InfiniteLoading/utils'
  ...
  methods: {
    ...
    */
    attemptLoad (isContinuousCall) {
      if (
        this.status !== STATUS.COMPLETE && isVisible(this.$el) && this.getCurrentDistance() <= this.distance
      ) {
